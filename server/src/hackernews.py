...
class HackerNews:
    ...
    def scrape_data(self) -> None:
        ...
        for attempt in range(1, self.RETRY_ATTEMPT + 1):
            try:
                ...
                for content in self.page.query_selector_all(
                    "//div[@id='content']//div[@class='blog-posts clear']/div"
                ):
                    link = content.wait_for_selector("xpath=./a").get_attribute("href")
                    img = (
                        content.wait_for_selector("xpath=.//div[@class='img-ratio']")
                        .inner_html()
                        .split('src="')[1]
                        .split('"')[0]
                    )
                    title = content.wait_for_selector(
                        "xpath=.//h2[@class='home-title']"
                    ).text_content()
                    date = (
                        content.wait_for_selector("xpath=.//span[@class='h-datetime']")
                        .inner_html()
                        .split("/i>")[-1]
                    )
                    category = content.wait_for_selector(
                        "xpath=.//span[@class='h-tags']"
                    ).inner_html()
                    description = content.wait_for_selector(
                        "xpath=.//div[@class='home-desc']"
                    ).text_content()
    ...
    def fetch_detail(self, url: str) -> None:
        ...
        page_content = self.page_2.wait_for_selector(
            "xpath=//div[@id='articlebody']"
        ).inner_html()
        author = self.page_2.query_selector_all("//span[@class='author']")[
            1
        ].inner_text()
        self.page_2.close()
